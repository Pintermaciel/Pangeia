...
class TestFornecedoresQuery(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        logger.info("Configuração do banco de dados do fornecedor")
        # Configuração do banco de dados do fornecedor
    ...
    def setUp(self):
        ...
        self.service = FornecedoresQuery()
    ...
    def test_fetch_and_transform_data(self):
        ...
        fornecedores_list = self.service.fetch_and_transform_data()
        logger.info(f"Number of records fetched: {len(fornecedores_list)}")

        # Verificar que há registros
        self.assertGreater(len(fornecedores_list), 0)

        # Converter para DataFrame
        df = pd.DataFrame([fornecedor.to_dict() for fornecedor in fornecedores_list])
        df.to_csv('fornecedores.csv', index=False)
        logger.info("Dados salvos em fornecedores.csv")

        # Verificar se os dados foram salvos corretamente
        saved_df = pd.read_csv('fornecedores.csv')
